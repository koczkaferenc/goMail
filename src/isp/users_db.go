...
/********************************************************* */
// User:LoadByLoginData()
/********************************************************* */
func (u *User) LoadByLoginData(email string, password string) error {
	log.Printf("email=%s, password=%s", email, password)
	err := db.Db.QueryRow(
		"SELECT id, name, email, enabled, admin FROM users WHERE email = ? AND password = ?",
		email, password ).Scan(
			&u.Id, &u.Name, &u.Email, &u.Enabled, &u.Admin)
